...
Deixando um pouco de lado uma análise numérica (olhar os scores gerados) e fazendo uma análise mais humana das notícias que foram classificadas mais relevantes, os modelos se sairam muito bem achando as notícias que se encaixam nos assuntos da query. No geral, as notícias mais relevantes para cada query realmente abordam o assunto da query, o que mostra que os modelos conseguiram capturar a semântica das notícias e associá-las às queries de forma eficiente. Por exemplo, para a query "inflação e preços ao consumidor", as notícias mais relevantes abordam diretamente o tema da inflação e seus impactos nos preços ao consumidor, o modelo inferiu corretamente que a sigla IPCA está presente e tem forte correlação com preços ao consumidor, o que indica que os modelos conseguiram identificar a relação semântica entre as notícias e a query.

### Sobre os modelos
O modelo `sentence-transformers/paraphrase-multilingual-mpnet-base-v2` apresentou uma performance melhor na busca semântica, apresentando notícias mais relevantes para as queries em comparação com o modelo `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`. O modelo mpnet-base-v2 conseguiu capturar melhor as nuances semânticas das notícias, o que resultou em uma busca mais precisa. Por exemplo, lendo as notícias como um leitor, a notícia para a query "mudanças de taxas de juros" que possui maior relação é a de ID 11, a qual aborda mudança na taxa Selic. O modelo mpnet-base-v2 conseguiu identificar essa relação, enquanto o algoritmo MiniLM-L12-v2 não conseguiu identificar essa relação tão claramente, apresentando notícias menos relevantes para a query antes da principal notícia sobre o assunto. Apesar do aumento de espaço ocupado pelo modelo mpnet-base-v2, a melhoria na performance justifica a escolha desse modelo para a geração de embeddings. Grosso modo, ambos os modelos apresentaram uma performance boa, mas o modelo mpnet-base-v2 se destacou por apresentar notícias mais relevantes para as queries, o que é crucial para um motor de busca semântico eficiente.
